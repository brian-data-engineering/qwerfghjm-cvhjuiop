...
# Tennis specific markets from your notes
ALLOWED_GROUPS = {
    1,   # Tennis 1X2 (Match Winner) -> T:1 (W1), T:3 (W2)
    17,  # Total Games O/U -> T:9 (Over), T:10 (Under) + P line
    2,   # Tennis Game Handicap -> T:7 (P1), T:8 (P2) + P line
}
...
def get_pending_matches():
    try:
        ...
        # Get recently synced to avoid duplicates
...
async def fetch_match(session, match):
    ...
    # Tennis uses gr=0 to ensure we catch both match and set markets
    api_url = (
...
async def run():
    ...
    if not matches:
        print("✅ Tennis matches are fresh.")
        return

    print(f"📊 Pending: {len(matches)} tennis matches")
