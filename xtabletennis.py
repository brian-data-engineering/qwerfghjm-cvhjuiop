...
# Table Tennis specific markets from your notes
ALLOWED_GROUPS = {
    1,   # Winner (Match) -> T:1, T:3
    2,   # Handicap (Points/Games) -> T:7, T:8 + P line
    17,  # Totals (O/U) -> T:9, T:10 + P line
    8,   # Double Chance -> T:4, T:5, T:6
}
...
def get_pending_matches():
    try:
        ...
        # Check for recently synced matches
        ...
        # Fetch pending table tennis matches
...
async def fetch_match(session, match):
    ...
    # gr=0 ensures all table tennis sub-groups are returned
    api_url = (
...
async def run():
    ...
    if not matches:
        print("✅ Table Tennis matches are fresh.")
        return

    print(f"📊 Pending: {len(matches)} table tennis matches")
